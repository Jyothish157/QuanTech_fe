...
export class ShiftService {
  ...
  private saveToStorage(): void {
    ...
    localStorage.setItem(
      this.SWAP_REQUESTS_KEY,
      JSON.stringify(this.swapRequestsSignal())
    );
  }
  ...
  approveSwapRequest(swapId: string): void {
    const swapRequest = this.swapRequestsSignal().find(
      (s) => s.SwapID === swapId
    );
    ...
    this.shiftsSignal.update((list) =>
      list.map((shift) => {
        if (
          shift.EmployeeID === swapRequest.EmployeeID &&
          shift.Date === swapRequest.Date
        ) {
          return {
            ...shift,
    ...
    if (this.shiftsSignal().length <= 3) {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        shiftTypes.forEach((shiftType) => {
          ...
          const shuffledEmployees = [...employeeIds].sort(
            () => 0.5 - Math.random()
          );
